...
import fileinput
statements=[]
with open("Question_3_assembler_input.txt",'r') as reader:
...
def typeA(statements):
    opcode=opcodes[statements[0]]
    reg1=registers[statements[1]]
    reg2=registers[statements[2]]
    reg3=registers[statements[3]]
    return opcode +'0'*2+reg1+reg2+reg3

def typeB (statements):
    opcode=opcodes[statements[1]]
    reg1=registers[statements[1]]
    imm=DecimalToBinary(int[statements[2]])
    return opcode+'0'*5+reg1+imm
...
for i in range(len(statements)):
    ...
    elif statements[i][0] in ["addf","subf"]:
        file_output.write(typeA(statements[i])+'\n')

    elif statements[i][0]=="movf":
        file_output.write(typeB(statements[i])+'\n')
